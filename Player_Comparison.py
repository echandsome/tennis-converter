...
def process_file(input_path, oo_threshold, uu_threshold):
    ...
    result_label.config(
        text=f"Saved:\nALL → {os.path.basename(all_path)}\nOO → {os.path.basename(oo_path)}\nUU → {os.path.basename(uu_path)}",
        fg="green"
    )
...
def run_conversion():
    ...
    root.update()
    try:
        process_file(path, oo_threshold, uu_threshold)
