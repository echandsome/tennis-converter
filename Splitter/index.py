...
def split_by_consecutive_groups(file_path, column_letter):
    ...
    for _, row in df.iterrows():
        ...
        if value != current_value:
            if group:
                save_group(group, current_value, file_count, output_dir, ext)
                file_count += 1
            group = [row]
            current_value = value
        else:
...
def save_group(group, value, count, output_dir, ext):
    group_df = pd.DataFrame(group)
    safe_value = str(value).strip().replace(" ", "_").replace("/", "_").replace("\\", "_")
    filename = f"{count:03d}_{safe_value}{ext}"
...
def run_split():
    ...
    root.update()
    try:
        split_by_consecutive_groups(file_path, column_letter)
    except Exception as e:
        result_label.config(text=f"Error: {str(e)}", fg="red")

root = tk.Tk()
root.title("File Splitter by Consecutive Groups")
...
tk.Label(root, text="Enter Column Letter to Split By (e.g., Q):").pack(pady=(10, 0))
